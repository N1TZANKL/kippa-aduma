...
import {
    lighten, withStyles, WithStyles, Theme, createStyles, WithTheme,
...
function Sidebar({ classes }: WithStyles<typeof styles>) {
    return (
        <Card className={classes.sidebar}>
            <div>
                {routes.map(route => <SidebarBox key={route.path} route={route} />)}
            </div>
            <IconButton title="Settings" className={classes.iconButton}><SettingsIcon className={classes.icon} /></IconButton>
        </Card>
    );
}
...
function SidebarBoxComponent(props: SidebarBoxProps) {
    ...
    return (
        <Link href={route.path}>
            ...
            >
                <SvgIcon className={classes.sidebarIcon} style={route.iconStyle}><path d={route.icon} /></SvgIcon>
            </div>
        </Link>
    );
}
